...
왼쪽 영역에서 프로젝트 기본 설정을, 오른쪽 영역에서 필요한 라이브러리를 추가할 수 있다.  
...
압축을 풀때는 여기에 압축 풀기를 선택하여 firstproject 만 생성 되도록 한다.  
( 윈도우에서 압축을 풀면 firstproject/firstproject로 생성이 됨. )
...
edu github에 있는 settings.xml 화일을 내용을 복사하여 아래 처럼 생성한다.( 보안사 삭제)  
